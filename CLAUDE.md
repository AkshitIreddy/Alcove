...
- Visual regression: `npm run visual` (`shots-now/visual-suite.mjs`) walks 16 surfaces × 2 window sizes × light/dark against the committed pictures in `qa/baseline/**` and writes a triptych per failure to `qa/baseline/__report/index.html`; a comparison run NEVER writes a baseline — `npm run visual -- --update` is the deliberate yes, `--only=<substring>` narrows it, and the whole matrix is ~30–45 min against the dev server on :1420.
