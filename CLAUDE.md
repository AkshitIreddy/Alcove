...
- `art/bookDesign.ts` — a book's **binding**: 50 spine shapes × 50 materials × 50 decorations, 189 presets picked deterministically from the book's seed. `drawBookSpine` replaces `flatShelf.drawSpine` inside `renderSpine`. **It reads no `flatScheme()` and must not start**: a book keeps its own colours in every room, which is what lets the reader recognise it. (`flatShelf.drawSpine` still exists for `drawCaseCard`/`drawBookRow` at card scale.) Every entry declares a `group` and a `tier` (signature / shelf / niche / oddity) and TypeScript refuses one that does not — the exported order is DERIVED from them, and `presetForSeed` rolls only `ROLLABLE_PRESETS`, so a reader is never handed an oddity while the studio still offers all of them.
...
**Routine testing only unless asked otherwise.** Default: `npx tsc --noEmit` and `npx vitest run` on the code you touched. Playwright E2E, the full visual matrix, probe scripts, and demo frame review are **in-depth** gates — run them only when the owner explicitly asks for that level of testing, not on every change.
...
- **The frame review is the strongest test this repo has, and it is the owner's stated preference: *"what is important is for you to visually verify the UI and visuals of the things as you do it, and for that to be the testing mechanism."*** `scripts/demo-sheets.mjs` tiles the recorded demo into numbered 4×4 boards and prints the frame number of every tile; `--frame=NNNN` and `--range=A-B` pull single frames back at full size. Point an agent per board at them, tell it to LOOK rather than to reason, and make a second agent pull the named frame and try to REFUTE each finding. On its first run over 1397 frames that found 26 confirmed defects against 18 refuted — content duplicating on read, the shelf blanking for half a second, a case repainting in two halves, every book losing its art, a key cap breaking across a line, a footnote overprinted by a callout — **none of which any of the 2729 unit tests could see, and most of which nobody had reported.** The refutations matter as much as the findings: the pass is only worth trusting because things were thrown out of it.
- **A gate you have not watched fail is not a gate.** Two examples from one week. A probe guarded the blank shelf with `colours <= 3`; genuinely blank frames scored 140 and 152, because the measured rectangle included chrome that survives — its verdict line said "no blank world" while asserting nothing. And a CLI fix was proved by a test that called the parser directly, so it passed while the shipped binary still printed the old error. Before trusting a new check, break the thing on purpose and watch the check go red — `probe-studio-repaint.mjs --sabotage` exists for exactly that, and it prints `GATE ALIVE` / `GATE INERT` rather than silently passing.
...
- **Installer QA runs SILENTLY. This machine is somebody's desktop, not a runner.** NSIS installers are GUI programs, so an install/uninstall verification loop throws a real window onto the owner's screen every iteration — which is exactly what happened, because a workflow prompt said "LOOK at the installer running before calling it done" and the agent obeyed it once per variation. Use `Alcove_<version>_x64-setup.exe /S` and `%LOCALAPPDATA%\Alcove\uninstall.exe /S`; every assertion worth making is a filesystem or registry read (`%LOCALAPPDATA%\Alcove` contents, `HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Alcove`, whether the library under `%APPDATA%` survived) and reads identically either way. Note that `/S` also skips the custom uninstall page, which makes it the test that the **default is to keep the library** — the one outcome that must never depend on a click.
  To judge how the installer LOOKS, open `src-tauri/icons/installer-header.bmp` and `installer-sidebar.bmp` — those two bitmaps plus NSIS's own chrome are the whole appearance, and looking at them costs no window at all. Run the GUI at most **once**, deliberately, and never in a loop.
- **Never install, uninstall or reinstall the app on this machine without saying so first.** The owner uses it. A silent uninstall is still an uninstall.
