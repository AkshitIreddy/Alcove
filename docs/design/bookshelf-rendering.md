...
## Recommendation
Hybrid: PixiJS v8 (WebGL) single-canvas world renderer with pre-baked flat sprite textures + a thin SolidJS DOM overlay for the pulled-out/opened book and HUD. No live SVG filters anywhere in the hot path.

(The world is the app's FLAT half — flat colour, one ink, bowed edges, per `src/art/flat.ts`. Two passages further down still say "hand-drawn", and they are left standing: both describe the pencil-and-watercolour spine pipeline that `RESET-render-architecture.md` deleted, where the word was accurate. Everything about the *renderer* here still holds.)
