...
## 5. Verification
Run `npx tsc --noEmit` and `npm test`. The smoke suite retains the parser's
totality invariant; malformed input must still return a document plus located
warnings rather than throw.
...
- **`npm run spec:check` is the generated-file check.** It regenerates from the same two inputs and fails if either checked-in file differs, printing the first differing lines and `Run: npm run spec`.

Net effect: adding an insertable is "add the name, write one line of prose (the compiler insists), run `npm run spec`". `npm run spec:check` catches a forgotten regeneration.
...
vitest ^4 (dev — the retained smoke suite)
