...
open with a module docstring — <!--f:docstringLines-->3841<!--/f--> lines in total,
...
| `npx vitest run` | <!--f:unitTests-->50<!--/f--> unit-test files, node environment (jsdom is deliberately not installed). `tests/book-bindings.test.ts` takes ~110 s on its own; that is expected. |
