 ...
 *   (creating it if needed) and flips forward when the cursor was carried.
 *   A target that already has a mounted editor is handed the blocks as a
 *   TRANSACTION on that editor — never as a document rewrite plus a remount,
 *   which is what made merely reading a book duplicate its content. The long
 *   note on `carryIntoLiveEditor` is the one to read before touching this.
 *
...
export default function BookView(): JSX.Element {
  ...
  /**
   * Hand carried blocks to a page that HAS A MOUNTED EDITOR, as a transaction
   * on that editor — never by rewriting its stored document underneath it.
   *
   * THIS IS THE FIX FOR READING A BOOK DUPLICATING IT. The store is not the
   * authority for a page with a live editor, and treating it as one corrupted
   * the book on nothing more than an open:
   *
   *   Both leaves of a spread mount inside ONE synchronous Solid effects flush
   *   (BookView builds left then right), and each runs its initial drain there
   *   — so by the end of that task the RIGHT leaf's editor has already peeled
   *   its own tail X off and handed X up through `onOverflow`. That removal
   *   reaches the store only through PageEditor's `queueMicrotask(mirror)`,
   *   which has not run yet. The LEFT leaf's carry, queued on `carryChain`
   *   ahead of it, therefore read `pages()[slot + 1]` and saw the right page's
   *   PRE-DRAIN document, X still in it. `prependBlocksToDoc` put X BACK,
   *   `updatePageDoc` restored it to the store and `savePageDoc` wrote it to
   *   the row — while X was simultaneously in flight to the page after that in
   *   the right leaf's own already-committed carry. Then `bumpDocVersion`
   *   remounted the right leaf on that resurrected document, its fresh editor
   *   drained X a second time, and the page after it received X TWICE.
   *   (The remount also destroyed the old editor before its drain could be
   *   serialized at all: Solid disposes a component's owned computations —
   *   where `createTiptapEditor` registers `instance.destroy()` — before the
   *   component's own `onCleanup(flushSave)`, so `mirror` bailed on
   *   `isDestroyed` and the removal was lost from both store and row.)
   *
   *   Measured with scripts/probe-page-duplication.mjs, before this: opening
   *   the Welcome book and turning through it took 0 duplicated blocks to 5
   *   across pages and 2 twice on one page. The first of them appeared with
   *   ZERO turns — it was never a page-turn bug, it was the two leaves of the
   *   very first spread.
   *
   * Going through the editor removes all three failures at once: the live
   * document already reflects the target's own drain so nothing can be
   * resurrected, there is no remount so no editor is destroyed mid-flight and
   * no second drain of the same content happens, and the page's row keeps
   * exactly ONE writer (its own debounced save) instead of two racing ones.
   * The insert re-enters the target's `extractOverflow` synchronously, so a
   * cascade still flows onward and the no-scrollbars contract is untouched.
   *
   * `addToHistory: false` for the same reason the drain and `anchorFreeMarks`
   * use it: a page break is not an edit the reader made, and one Ctrl+Z must
   * not half-undo one.
   *
   * @returns true when the live editor took the blocks; false when the page
   *          has no mounted editor — or the schema refused the JSON, in which
   *          case the caller's document rewrite is still better than dropping
   *          content on the floor.
   */
  const carryIntoLiveEditor = (
    pageId: string,
    blocks: readonly unknown[],
  ): boolean => {
    if (blocks.length === 0) return false;
    const instance = getPageEditor(pageId);
    if (!instance || !instance.view.dom.isConnected) return false;

    let nodes;
    try {
      nodes = blocks.map((json) => instance.schema.nodeFromJSON(json));
    } catch {
      return false;
    }

    // Same rule as `prependBlocksToDoc`, asked of the LIVE document rather
    // than restated against ProseMirror's node API: a blank target is
    // REPLACED, not prepended to, or every carry into a fresh page leaves the
    // starter empty paragraph stranded between batches. Reusing
    // `docHasContent` costs one doc walk and cannot drift from the other path.
    const blank = !docHasContent(instance.getJSON() as PageDoc);
    const tr = instance.state.tr;
    if (blank) tr.replaceWith(0, instance.state.doc.content.size, nodes);
    else tr.insert(0, nodes);
    instance.view.dispatch(tr.setMeta('addToHistory', false));

    // Publish the merged document to the store in THIS task rather than
    // waiting for the editor's own mirror microtask. Nothing in the carry
    // chain should ever have to reason about how far behind the store is —
    // that lag is the whole bug above.
    updatePageDoc(pageId, instance.getJSON() as PageDoc);
    return true;
  };
  ...
  const carryOverflow = async (
    ...
    // A mounted target owns its own document (see carryIntoLiveEditor). Only
    // a page with NO editor — the ordinary case, the next spread's left leaf,
    // or a sheet appended a moment ago — is addressed through the store, and
    // there is nothing live for that stored doc to be stale against.
    if (!carryIntoLiveEditor(next.id, kept)) {
      const merged = prependBlocksToDoc(next.doc, kept, fallbackAttrs);
      updatePageDoc(next.id, merged);
      bumpDocVersion(next.id); // the leaf is not on this spread; no editor dies
