...
export const TITLE_PLATES = [
  ...
  // Second curation: these append-only ids are the authored production
  // catalogue. The preceding archive remains readable only for migration.
  'direct-blind-title',
  'direct-gilt-title',
  'direct-ink-title',
  'press-small-caps',
...
const COVER_TITLE_TREATMENT_SPECS: Readonly<Record<TitlePlateStyle, TitlePlateSpec>> = {
  ...
  /* ---- second curation: accepted only through rendered comparison ---- */

  'direct-blind-title': plate('direct-blind-title', 'Blind title', 'The title and two short rules impressed directly into the covering.',
    ['plain', 'severe', 'antique'],
    { ground: 'none', shape: 'rect', frame: 'none', frameInk: 'soft', ends: 'rule', letter: 'soft', grain: 'none', outline: 0, radius: 0, seat: 'sunk' }),
  'direct-gilt-title': plate('direct-gilt-title', 'Gilt title', 'Gold lettering struck directly onto the covering with one finishing rule.',
    ['gilded', 'formal', 'plain'],
    { ground: 'none', shape: 'rect', frame: 'none', frameInk: 'gilt', ends: 'rule', letter: 'gilt', grain: 'none', outline: 0, radius: 0 }),
  'direct-ink-title': plate('direct-ink-title', 'Inked title', 'A dark or pale ink title printed directly on cloth without a label.',
    ['plain', 'modern', 'utilitarian'],
    { ground: 'none', shape: 'rect', frame: 'none', frameInk: 'ink', ends: 'none', letter: 'ink', grain: 'none', outline: 0, radius: 0 }),
  'press-small-caps': plate('press-small-caps', 'Dentelle title field', 'A wide title field joined to open perimeter-like dentelle returns.',
    ['scholarly', 'plain', 'formal'],
    { ground: 'none', shape: 'rect', frame: 'none', frameInk: 'ink', ends: 'double-rule', letter: 'ink', grain: 'none', outline: 0, radius: 0 }),
  'printer-floret-imprint': plate('printer-floret-imprint', 'Grolier nested field', 'A nested rounded-rectangle field with one small central printer’s flower.',
    ['scholarly', 'refined', 'antique'],
    { ground: 'none', shape: 'rect', frame: 'none', frameInk: 'gilt', ends: 'fleuron', letter: 'auto', grain: 'none', outline: 0, radius: 0 }),

  'laid-paper-ticket': plate('laid-paper-ticket', 'Laid paper ticket', 'A square hand-ruled paper label pasted flat to the board.',
    ['plain', 'scholarly', 'cosy'],
    { ground: 'cream', shape: 'rect', frame: 'single', frameInk: 'ink', ends: 'none', letter: 'ink', grain: 'none', outline: 0.7, radius: 0.01 }),
  'deckled-paper-ticket': plate('deckled-paper-ticket', 'Vertical paper slip', 'A tall deckled paper slip pasted near the fore-edge and read vertically.',
    ['natural', 'rustic', 'cosy'],
    { ground: 'creamDeep', shape: 'rect', frame: 'none', frameInk: 'soft', ends: 'rule', letter: 'soft', grain: 'none', outline: 0.65, radius: 0 }),
  'vellum-rule-ticket': plate('vellum-rule-ticket', 'Vellum shoulder band', 'A broad vellum band laid high across the board with a fine double keyline.',
    ['pale', 'antique', 'refined'],
    { ground: 'creamDeep', shape: 'rect', frame: 'double', frameInk: 'ink', ends: 'none', letter: 'ink', grain: 'none', outline: 0.7, radius: 0.01 }),
  'parchment-slip': plate('parchment-slip', 'Vertical parchment slip', 'A narrow warm parchment slip sewn near the joint and read vertically.',
    ['pale', 'antique', 'scholarly'],
    { ground: 'timber', shape: 'rect', frame: 'brackets', frameInk: 'soft', ends: 'none', letter: 'ink', grain: 'none', outline: 0.65, radius: 0.01 }),

  'morocco-single-rule': plate('morocco-single-rule', 'Morocco single rule', 'A dark goatskin lettering piece edged by one gilt rule.',
    ['formal', 'gilded', 'refined'],
    { ground: 'plum', shape: 'rect', frame: 'single', frameInk: 'gilt', ends: 'none', letter: 'gilt', grain: 'none', outline: 0.9, radius: 0.01, seat: 'sunk' }),
  'morocco-double-rule': plate('morocco-double-rule', 'Morocco double rule', 'A rectangular goatskin label with two unequal gilt keylines.',
    ['formal', 'gilded', 'antique'],
    { ground: 'plum', shape: 'rect', frame: 'double', frameInk: 'gilt', ends: 'rule', letter: 'gilt', grain: 'none', outline: 0.9, radius: 0.01, seat: 'sunk' }),
  'morocco-clipped-rule': plate('morocco-clipped-rule', 'Grolier lozenge', 'A true leather lozenge title locus ruled once in gilt.',
    ['formal', 'refined', 'antique'],
    { ground: 'plum', shape: 'octagon', frame: 'single', frameInk: 'gilt', ends: 'none', letter: 'gilt', grain: 'none', outline: 0.9, radius: 0.01, seat: 'sunk' }),
  'calf-blind-label': plate('calf-blind-label', 'Oval calf compartment', 'A central oval title locus held inside a broad calf compartment.',
    ['warm', 'antique', 'formal'],
    { ground: 'terracotta', shape: 'rect', frame: 'double', frameInk: 'soft', ends: 'rule', letter: 'gilt', grain: 'none', outline: 0.85, radius: 0.01, seat: 'sunk' }),
  'two-tone-leather-label': plate('two-tone-leather-label', 'Inset leather centre', 'A large flush leather centre panel let into a contrasting surround.',
    ['formal', 'warm', 'refined'],
    { ground: 'ink', shape: 'rect', frame: 'double', frameInk: 'gilt', ends: 'none', letter: 'cream', grain: 'none', outline: 0.9, radius: 0.01, seat: 'sunk' }),
  'library-buckram-label': plate('library-buckram-label', 'Library buckram label', 'A compact cloth label with square press rules and printed lettering.',
    ['scholarly', 'utilitarian', 'plain'],
    { ground: 'slate', shape: 'rect', frame: 'single', frameInk: 'cream', ends: 'rule', letter: 'cream', grain: 'none', outline: 0.7, radius: 0.01, seat: 'flush' }),

  'dyed-leather-crossband': plate('dyed-leather-crossband', 'Half-leather title field', 'A broad split-material field occupying the upper third of the board.',
    ['warm', 'antique', 'plain'],
    { ground: 'plum', shape: 'rect', frame: 'none', frameInk: 'soft', ends: 'double-rule', letter: 'gilt', grain: 'none', outline: 0, radius: 0, seat: 'flush' }),
  'gilt-ruled-crossband': plate('gilt-ruled-crossband', 'Gilt ruled crossband', 'A dark leather band bounded by one heavy and one hairline gilt rule.',
    ['gilded', 'formal', 'heavy'],
    { ground: 'ink', shape: 'rect', frame: 'none', frameInk: 'gilt', ends: 'double-rule', letter: 'gilt', grain: 'none', outline: 0, radius: 0, seat: 'flush' }),
  'cloth-inlay-crossband': plate('cloth-inlay-crossband', 'Vertical cloth inlay', 'A narrow contrasting cloth inlay running vertically near the fore-edge.',
    ['modern', 'plain', 'refined'],
    { ground: 'slate', shape: 'rect', frame: 'none', frameInk: 'cream', ends: 'rule', letter: 'cream', grain: 'none', outline: 0, radius: 0, seat: 'flush' }),
  'split-leather-crossband': plate('split-leather-crossband', 'Split-board title field', 'Two broad leather rails define a central title field across split boards.',
    ['formal', 'antique', 'refined'],
    { ground: 'terracotta', shape: 'rect', frame: 'none', frameInk: 'gilt', ends: 'double-rule', letter: 'cream', grain: 'none', outline: 0, radius: 0, seat: 'flush' }),

  'oxford-blind-compartment': plate('oxford-blind-compartment', 'Oxford blind compartment', 'Open blind fillets step inward at the head and tail.',
    ['scholarly', 'severe', 'antique'],
    { ground: 'none', shape: 'stepped', frame: 'double', frameInk: 'soft', ends: 'none', letter: 'soft', grain: 'none', outline: 0, radius: 0, seat: 'sunk' }),
  'cambridge-calf-compartment': plate('cambridge-calf-compartment', 'Arts & Crafts blind title', 'Asymmetric blind corner returns and direct lettering in the Kelmscott manner.',
    ['scholarly', 'warm', 'antique'],
    { ground: 'none', shape: 'rect', frame: 'double', frameInk: 'soft', ends: 'bracket', letter: 'gilt', grain: 'none', outline: 0, radius: 0 }),
  'french-triple-fillet': plate('french-triple-fillet', 'Perimeter title bridge', 'Triple gilt fillets extend laterally toward the cover’s perimeter frame.',
    ['formal', 'gilded', 'refined'],
    { ground: 'none', shape: 'rect', frame: 'triple', frameInk: 'gilt', ends: 'none', letter: 'gilt', grain: 'none', outline: 0, radius: 0 }),
  'ledger-open-field': plate('ledger-open-field', 'Armorial inscription field', 'A formal square inscription field with an open pediment and ruled foot.',
    ['utilitarian', 'scholarly', 'plain'],
    { ground: 'none', shape: 'rect', frame: 'none', frameInk: 'ink', ends: 'double-rule', letter: 'ink', grain: 'none', outline: 0, radius: 0 }),
  'inscription-shoulders': plate('inscription-shoulders', 'Lobed Islamic cartouche', 'A restrained six-lobed inscription cartouche tooled directly into leather.',
    ['formal', 'antique', 'refined'],
    { ground: 'none', shape: 'rect', frame: 'brackets', frameInk: 'gilt', ends: 'none', letter: 'gilt', grain: 'none', outline: 0, radius: 0 }),
...
export const ACTIVE_TITLE_PLATES = [
  // Persisted absence sentinel. It is reader-selectable but excluded from the
  // 25-art specimen catalogue; it carries no physical title furniture.
  'none',
  'direct-blind-title',
  'direct-gilt-title',
  'direct-ink-title',
...
const RETIRED_TITLE_PLATE_REPLACEMENTS: Readonly<Partial<Record<TitlePlateStyle, TitlePlateStyle>>> = {
  gilt: 'direct-gilt-title',
  label: 'laid-paper-ticket',
  debossed: 'direct-blind-title',
  'morocco-label': 'morocco-single-rule',
  'double-fillet': 'french-triple-fillet',
  'triple-fillet': 'french-triple-fillet',
  'blind-panel': 'oxford-blind-compartment',
  'gilt-cartouche': 'inscription-shoulders',
  'paper-slip': 'laid-paper-ticket',
  'ruled-box': 'ledger-open-field',
  roundel: 'inscription-shoulders',
  'lozenge-plate': 'inscription-shoulders',
  'shield-plate': 'inscription-shoulders',
  'scroll-plate': 'inscription-shoulders',
  'stone-tablet': 'renaissance-title-window',
  'oval-medallion': 'inscription-shoulders',
  'bead-frame': 'french-triple-fillet',
  'rope-frame': 'cambridge-calf-compartment',
  'dotted-rule': 'ledger-open-field',
  'corner-brackets': 'cambridge-calf-compartment',
  'notched-corners': 'morocco-clipped-rule',
  'scallop-edge': 'deckled-paper-ticket',
  'sunk-panel': 'oxford-blind-compartment',
  'leather-onlay': 'two-tone-leather-label',
  'inlay-strip': 'cloth-inlay-crossband',
  'vellum-slip': 'vellum-rule-ticket',
  'linen-tag': 'library-buckram-label',
  'gilt-band': 'gilt-ruled-crossband',
  'twin-rules': 'ledger-open-field',
  'ribbon-band': 'dyed-leather-crossband',
  'blind-lettered': 'direct-blind-title',
  'gilt-direct': 'direct-gilt-title',
  'ink-panel': 'library-buckram-label',
  'ivory-plate': 'vellum-rule-ticket',
  'ebony-plate': 'two-tone-leather-label',
  'copper-plate': 'calf-blind-label',
  'enamel-plate': 'library-buckram-label',
  'crest-plate': 'renaissance-title-window',
  'arched-plate': 'renaissance-title-window',
  pedimented: 'renaissance-title-window',
  'chamfered-plate': 'morocco-clipped-rule',
  'stepped-frame': 'oxford-blind-compartment',
  'hatched-ground': 'cambridge-calf-compartment',
  'stippled-ground': 'cambridge-calf-compartment',
  'wreathed-plate': 'inscription-shoulders',
  'starred-ends': 'printer-floret-imprint',
  'fleuron-ends': 'printer-floret-imprint',
  'lozenge-ends': 'press-small-caps',
  'gothic-panel': 'renaissance-title-window',
  'oxford-compartment': 'oxford-blind-compartment',
  'calf-compartment': 'calf-blind-label',
  'vellum-ink-field': 'vellum-rule-ticket',
  'printer-imprint': 'press-small-caps',
  'split-binding-band': 'dyed-leather-crossband',
  'presentation-shoulder': 'morocco-clipped-rule',
};
