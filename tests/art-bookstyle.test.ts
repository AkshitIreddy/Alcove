...
/*
 * `bindingMaterialSlug` had a suite here, checking that every binding mapped
 * onto a sheet that actually shipped in the generated material library. That
 * library (`art/materials.ts`, and the brush engine that tinted its tiles) is
 * gone, so there is nothing for a slug to name. The function itself is still
 * exported from `art/spines.ts` with no caller left — one for whoever owns
 * that file.
 */
